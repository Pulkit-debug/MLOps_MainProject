#!/usr/bin/env python
# coding: utf-8
# Testing the file after a long time
...
def addingTopModel(bottom_model, num_classes):
...
FC_Head = addingTopModel(MobileNet, num_classes)
#num_classes is how many outputs we want to use
...
train_data_dir = '/mlops/train'
validation_data_dir = '/mlops/validation'
...
checkpoint = ModelCheckpoint("houseRecog.h5",
                             monitor="val_loss",
                             mode="min",
                             save_best_only = True,
                             verbose=1)
...
classifier = load_model('houseRecog.h5')
...
def draw_test(name, pred, im):
    ...
    cv2.putText(expanded_image, newHouse, (20, 60) , cv2.FONT_HERSHEY_SIMPLEX,1, (0,0,255), 2)
...
for i in range(0,10):
    input_im = getRandomImage("/mlops/validation/")
